...
static const char *CTAG = "custom_connect";

#if CONFIG_EXAMPLE_CONNECT_WIFI
static esp_netif_t *wifi_start(char* PARAMSSID, char* PARAMPASS);
 ...
 * All netifs created withing common connect component are prefixed with the module CTAG,
...
static void start(char* PARAMSSID, char* PARAMPASS)
{

#if CONFIG_EXAMPLE_CONNECT_WIFI
    s_example_esp_netif = wifi_start(PARAMSSID, PARAMPASS);
    s_active_interfaces++;
#endif
                      ...
                      int32_t event_id, void *event_data)
{
    ...
    if (!is_our_netif(CTAG, event->esp_netif)) {
        ESP_LOGW(CTAG, "Got IPv4 from another interface \"%s\": ignored", esp_netif_get_desc(event->esp_netif));
        return;
    }
    ESP_LOGI(CTAG, "Got IPv4 event: Interface \"%s\" address: " IPSTR, esp_netif_get_desc(event->esp_netif), IP2STR(&event->ip_info.ip));
                        ...
                        int32_t event_id, void *event_data)
{
    ...
    if (!is_our_netif(CTAG, event->esp_netif)) {
        ESP_LOGW(CTAG, "Got IPv6 from another netif: ignored");
        return;
    }
    esp_ip6_addr_type_t ipv6_type = esp_netif_ip6_get_addr_type(&event->ip6_info.ip);
    ESP_LOGI(CTAG, "Got IPv6 event: Interface \"%s\" address: " IPV6STR ", type: %s", esp_netif_get_desc(event->esp_netif),
...
esp_err_t custom_connect(char* PARAMSSID, char* PARAMPASS)
{
#if EXAMPLE_DO_CONNECT
...
#endif
    start(PARAMSSID, PARAMPASS);
    ESP_ERROR_CHECK(esp_register_shutdown_handler(&stop));
    ESP_LOGI(CTAG, "Waiting for IP(s)");
    ...
    for (int i = 0; i < esp_netif_get_nr_of_ifs(); ++i) {
        ...
        if (is_our_netif(CTAG, netif)) {
            ESP_LOGI(CTAG, "Connected to %s", esp_netif_get_desc(netif));
            ESP_ERROR_CHECK(esp_netif_get_ip_info(netif, &ip));

            ESP_LOGI(CTAG, "- IPv4 address: " IPSTR, IP2STR(&ip.ip));
#ifdef CONFIG_EXAMPLE_CONNECT_IPV6
            esp_ip6_addr_t ip6[MAX_IP6_ADDRS_PER_NETIF];
            int ip6_addrs = esp_netif_get_all_ip6(netif, ip6);
            for (int j = 0; j < ip6_addrs; ++j) {
                esp_ip6_addr_type_t ipv6_type = esp_netif_ip6_get_addr_type(&(ip6[j]));
                ESP_LOGI(CTAG, "- IPv6 address: " IPV6STR ", type: %s", IPV62STR(ip6[j]), s_ipv6_addr_types[ipv6_type]);
            }
#endif
...
static void on_wifi_disconnect(void *arg, esp_event_base_t event_base,
                               int32_t event_id, void *event_data)
{
    ESP_LOGI(CTAG, "Wi-Fi disconnected, trying to reconnect...");
...
static esp_netif_t *wifi_start(char* PARAMSSID, char* PARAMPASS)
{
    ...
    // Prefix the interface description with the module CTAG
    // Warning: the interface desc is used in tests to capture actual connection details (IP, gw, mask)
    asprintf(&desc, "%s: %s", CTAG, esp_netif_config.if_desc);
    ...
    STA.scan_method = EXAMPLE_WIFI_SCAN_METHOD;
    STA.sort_method = EXAMPLE_WIFI_CONNECT_AP_SORT_METHOD;
    STA.threshold.rssi = CONFIG_EXAMPLE_WIFI_SCAN_RSSI_THRESHOLD;
    STA.threshold.authmode = EXAMPLE_WIFI_SCAN_AUTH_MODE_THRESHOLD;
    // STA.ssid = PARAMSSID;
    wifi_config.sta = STA;

    memcpy(wifi_config.sta.ssid, PARAMSSID, 21);
    memcpy(wifi_config.sta.password, PARAMPASS, 21);

    ESP_LOGI(CTAG, "Connecting to %s...", wifi_config.sta.ssid);
                         ...
                         int32_t event_id, void *event_data)
{
    ...
    case ETHERNET_EVENT_CONNECTED:
        ESP_LOGI(CTAG, "Ethernet Link Up");
...
esp_netif_t *get_example_netif_from_desc(const char *desc)
{
    esp_netif_t *netif = NULL;
    char *expected_desc;
    asprintf(&expected_desc, "%s: %s", CTAG, desc);
