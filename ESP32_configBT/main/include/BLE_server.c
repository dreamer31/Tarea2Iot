...
{
    .attr_max_len = GATTS_DEMO_CHAR_VAL_LEN_MAX,//max char length 64 (bytes asumo)
...
void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param){
    switch (event) {
#ifdef CONFIG_SET_RAW_ADV_DATA
    case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT:/*For the case of raw advertising data set, the event triggered is ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT*/
        adv_config_done &= (~adv_config_flag);
        if (adv_config_done==0){
            esp_ble_gap_start_advertising(&adv_params);
        }
        break;
    case ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT:/*when the raw scan response data is set, ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT event is triggered.*/
...
void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: { /* To manage the write events*/
        if (!param->write.is_prep){
            ...
            newConfig = (char*) malloc(sizeof(char)*param->write.len);
            for(int i =0; i<rx_len; i++){
                newConfig[i]=param->write.value[i];
            }
    ...
    case ESP_GATTS_UNREG_EVT:
        break;
    case ESP_GATTS_CREATE_EVT: /*When a service is created successfully, an ESP_GATTS_CREATE_EVT event is triggered, and can be used to start the service and add characteristics*/
...
/** @brief Send data through the BLE protcol as notification or indication 
    ...
    *  @param  data An array woth the data to send 
    ...
    * */
esp_err_t BLE_send(uint8_t data_len, uint8_t *data, bool type){
    esp_err_t ret;
    ret = esp_ble_gatts_send_indicate(gl_profile_tab[PROFILE_A_APP_ID].gatts_if, 
